...
uint8_t* encode_image (uint8_t *input_ptr, uint8_t *output_ptr, uint32_t quality_factor, uint32_t image_format, uint32_t image_width, uint32_t image_height, uint8_t add_dri_header, int* parts)
{
    uint16_t i, j;
    uint8_t* last = output_ptr;
    parts[0] = 1;
    ...
    if (add_dri_header)
    {
      ...
      parts[1] = output_ptr - last; last = output_ptr;
      parts[0]++;
    }
    ...
    for (i=1; i<=jpeg_encoder_structure->vertical_mcus; i++)
    {
        ...
        for (j=1; j<=jpeg_encoder_structure->horizontal_mcus; j++)
        {
            ...
            parts[parts[0]++] = output_ptr - last; last = output_ptr;

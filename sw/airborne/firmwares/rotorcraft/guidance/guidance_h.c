...
static void guidance_h_traj_run(bool_t in_flight) {
  ...
  if (in_flight) {
    // guidance_h_pos_err_sum: no more pos error but trim angle
    guidance_h_pos_err_sum.x += ((guidance_h_igain * (guidance_h_pos_err.x)) >> (INT32_POS_FRAC + 4));
    guidance_h_pos_err_sum.y += ((guidance_h_igain * (guidance_h_pos_err.y)) >> (INT32_POS_FRAC + 4));
  ...
  guidance_h_command_earth.x += guidance_h_pos_err_sum.x;
  guidance_h_command_earth.y += guidance_h_pos_err_sum.y;

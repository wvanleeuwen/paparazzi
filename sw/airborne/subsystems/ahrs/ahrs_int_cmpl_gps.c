...
//
// gravity heuristic
// gps based gravity correction <-- working on that
// gps update for yaw on fixed wing ?
//

#include "subsystems/ahrs/ahrs_int_cmpl_gps.h"
...
void ahrs_update_accel(void) {
	...
	accel_float.x =   ACCEL_BUTTER_NUM_1*accel_float.x
	...
	accel_float.y =   ACCEL_BUTTER_NUM_1*accel_float.y
	...
	accel_float.z =   ACCEL_BUTTER_NUM_1*accel_float.z
	...
	accel_filter_val_prev		= accel_float;

	ACCELS_BFP_OF_REAL(accel_bfp_filtered,accel_float);
#else
	accel_bfp_filtered = imu.accel;
#endif

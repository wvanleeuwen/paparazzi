...
{
  intermcu_msg_send_RADIO_COMMANDS(&(intermcu_transport.trans_tx), intermcu_device,
    INTERMCU_FBW, RADIO_CONTROL_NB_CHANNEL, radio_control.values);
}
...
static inline void intermcu_parse_msg(struct transport_rx * trans, void (*commands_frame_handler)(void))
{
  ...
  switch (msg_id) {
    case MSG_INTERMCU_COMMANDS: {
        uint8_t i;
        uint8_t size = INTERMCU_COMMANDS_values_length(trans->payload);
        int16_t *new_commands = INTERMCU_COMMANDS_values(trans->payload);
        for(i = 0; i < size; i++)

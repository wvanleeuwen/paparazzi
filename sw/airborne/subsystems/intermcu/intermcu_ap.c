...
void intermcu_set_actuators(pprz_t *command_values, uint8_t ap_mode __attribute__((unused)))
{
  ...
  intermcu_msg_send_COMMANDS(&(intermcu_transport.trans_tx), intermcu_device,
    INTERMCU_AP, COMMANDS_NB, command_values);
}
...
static inline void intermcu_parse_msg(struct transport_rx * trans, void (*rc_frame_handler)(void))
{
  ...
  switch (msg_id) {
    case MSG_INTERMCU_RADIO_COMMANDS: {
        uint8_t i;
        uint8_t size = INTERMCU_RADIO_COMMANDS_values_length(trans->payload);
        int16_t *rc_values = INTERMCU_RADIO_COMMANDS_values(trans->payload);
        for(i = 0; i < size; i++)

...
static struct opticflow_state_t opticflow_state;  ///< State of the drone to communicate with the opticflow // todo change to state of camera
...
/* Static functions */
static struct image_t *opticflow_module_calc(struct image_t *img);     ///< The main optical flow calculation thread
...
/**
 * Initialize the optical flow module for the requested camera
 */
...
static struct image_t *opticflow_module_calc(struct image_t *img)
{
  ...
  float_rmat_transp_vmult(&opticflow_result.vel_body, orientationGetRMat_f(&(OPTICFLOW_CAMERA.body_to_cam)),
                          &opticflow_result.vel);
 ...
 */
// TODO this only considers try agl measurements, if not available can also estimate using baro
// assume height above ground = height above obstacle

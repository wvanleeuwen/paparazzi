...
static uint8_t *jpeg_huffman(JPEG_ENCODER_STRUCTURE *jpeg_encoder_structure, uint16_t component, uint8_t *output_ptr)
{
  ...
  uint16_t AbsCoeff, HuffCode, HuffSize, RunLength = 0, DataSize = 0, h_index;
  ...
  for (i = 63; i > 0; i--) {
    if ((Coeff = *Temp_Ptr++) != 0) {
      ...
      h_index = RunLength * 10 + DataSize;
      HuffCode = AcCodeTable [h_index];
      HuffSize = AcSizeTable [h_index];

 ...
 */
/**
 * @file "modules/computer_vision/wv_swarm/wv_swarm.cpp"
...
#include <opencv2/core.hpp>			// Core openCV functionality
#include <opencv2/imgproc.hpp> 		// openCV image processing library
#include "wv_swarm.h" 				// Own header
using namespace cv;
...
extern void 			bebop_front_camera_set_exposure(int exposure);

//static void 			enlargeRect			(Rect& sourceRect, Rect& destRect, float factor, Size frameSize);
...
int 	WV_TRACK_GREY_THRESHOLD 	= 60; 		// [TEST 20160602 in optitrack] [HOME: 120] [OLD ZOO: 140]
...
void wv_swarm_init(int globalMode)
{
	...
	bebop_front_camera_set_exposure(100);
	return;
}
...
char* wv_swarm_run(char* img, int width, int height)
{
	...
	Mat frameBGR(height, width, CV_8UC4, img); 				// Initialize current frame in openCV
	...
	frameBGR 	= frameBGR(crop);
	Mat frameGrey(frameBGR.rows, frameBGR.cols, CV_8UC1, cvScalar(0.0));
	trackGreyObjects(frameBGR, frameGrey, &trackRes, crop); 				// Track objects in frameBGR
#if WV_OPT_MOD_VIDEO
	vector<Mat> channels;
	Mat fullYUVFrame;
...
{
	pixCount++;
	if(destFrame.at<uint8_t>(row,col) > 0)
	{
		return false;
	}
	int B 		= sourceFrame.at<Vec4b>(row,col)[0];
	int G 		= sourceFrame.at<Vec4b>(row,col)[1];
	int R 		= sourceFrame.at<Vec4b>(row,col)[2];
	double gr 	= ( R * R - 0.9 * B * B - 0.8 * G * G) / 255;
...
void enlargeRect(Rect& sourceRect, Rect& destRect, float factor, Size frameSize)
{
	// This function enlarges a given rectangle by factor
	destRect.y	= sourceRect.y - 0.5 * (factor - 1) * sourceRect.height;
	destRect.x	= sourceRect.x - 0.5 * (factor - 1) * sourceRect.width;
	// Figure out correct y and height
	if(destRect.y < 0)
	{
		// Don't let the rectangle go outside the frame
		destRect.y 		= 0;
		destRect.height = (1 + (factor - 1) * 0.5) * sourceRect.height;
	}else{
		destRect.height = sourceRect.height * factor;
	}
	if(destRect.y+destRect.height > frameSize.height)
	{
		destRect.height = frameSize.height - destRect.y;
	}
	// Figure out correct x and width
	if(destRect.x < 0)
	{
		// Don't let the rectangle go outside the frame
		destRect.x 		= 0;
		destRect.width 	= (1 + (factor - 1) * 0.5) * sourceRect.width;
	}else{
		destRect.width  = sourceRect.width * factor;
	}
	if(destRect.x+destRect.width > frameSize.width)
	{
		destRect.width = frameSize.width - destRect.x;
	}
	return;
}
*/

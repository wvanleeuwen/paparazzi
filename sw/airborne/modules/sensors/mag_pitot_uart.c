...
static inline void mag_pitot_parse_msg(void)
{
  ...
  case DL_IMCU_REMOTE_BARO: {
    float baro_abs = 1.0f * ((float)(DL_IMCU_REMOTE_BARO_pitot_tot(mp_msg_buf)));
    float baro_diff = 1.0f * DL_IMCU_REMOTE_BARO_pitot_stat(mp_msg_buf);
    float baro_temp = 1.0f * DL_IMCU_REMOTE_BARO_pitot_temp(mp_msg_buf);

    AbiSendMsgBARO_ABS(IMU_MAG_PITOT_ID, baro_abs);
    AbiSendMsgBARO_DIFF(IMU_MAG_PITOT_ID, baro_diff);
    AbiSendMsgTEMPERATURE(IMU_MAG_PITOT_ID, baro_temp);
    break;
  }
